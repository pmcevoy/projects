...
`UnitProfile.LeadingAbilities` (`List<AbilityProfile>`) — populated at enrichment time by `Enricher.cs`, which filters abilities whose text starts with `"While this model is leading a unit"` into this separate list rather than `Abilities`. Displayed in `_UnitCard.cshtml` as a collapsible "While Leading" section styled in amber (`#f1a94e`), visually distinct from regular abilities. Not consumed by the simulation engine.
