...
Get-Content -Path $csvFilePath |
	...
	%{
		...
		$safeItemName = $itemName -replace '[\\/:*?"<>|]', '' #strip out invalid chars

		$src = "$($sourceRoot)\$($sourcePath)"
		$dest = "$($destRoot)\$($username)\$($itemDate)\$($safeItemName)-$($itemID)$($extension)"
		...
		if( !(Test-Path "$($destRoot)\$($username)\$($itemDate)") ){
			New-Item -Type Directory "$($destRoot)\$($username)\$($itemDate)\" | Out-Null
		}
